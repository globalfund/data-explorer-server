...
export const mapData = function (data, mapping, dataTypes, dimensions) {
  ...
  d3.rollups(
    data,
    (v) => {
      const countryField = v[0][mapping.country.value]
      let name = countryField
      if (countryField.length < 4) {
        const fCountry = countries.find(
          (v) => v.iso2 === countryField || v.iso3 === countryField
        )
        if (fCountry) {
          name = fCountry.country
        }
      }
      const item = {
        name,
    ...
    (d) => d[mapping.country.value].toString()
  )
